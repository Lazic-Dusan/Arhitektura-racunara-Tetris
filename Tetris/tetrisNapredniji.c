...
#define slice   (GAME_DATA->slice)
#define oldStart   (GAME_DATA->oldStart)
    ...
    while (1) {
        ...
        *vgaCont  = 0x3;
        ...
        while (!hit_bottom) {
            ...
            else if (req_dir == 7) { // HARD DROP (Space)
                int drop_points = 0;
                while (!check_collision_ext(cur_row + 1, cur_col, cur_w, cur_h)) {
                    cur_row++;
                    drop_points++;
                }
                points += drop_points * 2; // Bonus poeni za Hard Drop
                *hexData = dec_to_hex_display(points);
                hit_bottom = 1; // Figura je stigla do dna
            }
            
            // REDOVNO SPUŠTANJE ZA JEDAN RED
            if(delayCnt >= get_delay_from_points(points)){
                int brLoop = 0;
                do{
                    if (check_collision_ext(cur_row + 1, cur_col, cur_w, cur_h)) {
                        hit_bottom = 1;
                    ...
                    brLoop++;
                }while((slice&oldStart) && !hit_bottom);
        ...
        // Nakon zaključavanja figure na dnu, proveravamo i brišemo pune redove
        asm volatile ("csrc mstatus, %0" :: "r"(MSTATUS_MIE));
        clear_full_lines();
        asm volatile ("csrs mstatus, %0" :: "r"(MSTATUS_MIE));
    }
...
void rotate_grid_4x4(void) {
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            rotated_grid[c * 4 + (3 - r)] = current_grid[r * 4 + c];
        }
    }
}
